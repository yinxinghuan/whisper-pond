...
- 素材清单：`public/img/aigram.svg` 使用新版 AlterU 竖向 mark，作为右下角 28px 水印；其余视觉全部由 Three.js 几何、灯光、材质和 Cannon 物理生成。
...
- 球体 y<-7 时重置到上方 y=5 到 7，防止越界后永久消失；x 越界不主动拉回，以保留原版自然下落路径。
- 6 条挡板是 mass=0 的 Cannon Box static body，位置和旋转与 Three.js mesh 一致。
- 点按游戏区域、换色按钮、键盘 Space 或键盘 C 会循环切换调色盘，并更新 500 个 instanced color。
...
- 历史最高记录保留为兼容字段，不参与当前原版 physics demo 逻辑。
- 球体越界会自动回收，不作为失败。
