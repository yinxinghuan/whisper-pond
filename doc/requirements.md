...
《Physics Pond》是一款 60 秒 3D 物理导流小游戏：500 个彩色小球沿交错挡板滚落，玩家拖动挡板改变角度，把球导向底部发光目标槽得分。
...
- 每 12 秒切换一次目标槽，顺序为中槽 → 左槽 → 右槽循环；切换时目标槽在场景与 HUD 中同步高亮，并显示 0.72 秒目标提示。
- 玩家可拖动任意挡板改变角度；每条挡板角度限制在 -42° 到 +42°，拖动灵敏度为每 1 world unit 横向移动约 0.42 rad，松手后保持当前角度。
...
- 球落入当前目标槽得 5 分并增加 streak；落入非目标槽得 1 分并清空 streak。
...
- 再来一次按钮重置分数、倒计时、收集数量、streak、目标槽、挡板角度和球体位置；目标按钮可切换目标槽；返回首页回到开始页。
